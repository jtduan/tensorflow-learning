...
class DataIter():
    ...
    def generateRamdomNumber(self, len):
        ...
        cv2.putText(img, text,
                    (np.random.randint(1, 5), np.random.randint(24, 30)), font, 0.8 + np.random.randn() / 7,
                    (255, 255, 255), 2)
        # cv2.putText(img, text,
        #             (2, 26), font, 0.7,
        #             (255, 255, 255), 2)
    ...
    def next_batch(self, batch_size):
        ...
        return images, labels_spase, [self.lstm_len] * batch_size

# img, label = DataIter().generateRamdomNumber(3)
#
# print label
# winname = 'example'
